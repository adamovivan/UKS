...
def create_milestone(request, owner=None, repo=None):
    r = requests.post(API + 'repos/{0}/{1}/milestones'.format(owner, repo), params = request.POST)
...
def update_milestone(request, owner=None, repo=None, number=None):
    r = requests.patch(API + 'repos/{0}/{1}/milestones/{2}'.format(owner, repo, number), params = request.POST)
    if r.status_code == 200:
...
def delete_milestone(request, owner=None, repo=None, number=None):
    r = requests.delete(API + 'repos/{0}/{1}/milestones/{2}'.format(owner, repo, number), params = request.POST)
    if r.status_code == 200:
